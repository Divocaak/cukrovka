...
if (!isset($_POST["battleId"])) {
    ...
    $battle = $player->get_casual_battle($link, false, $_POST["battleId"])[0];
    $defenderId = $battle->attacker->id;
...
<body>
    <div class="row m-5">
        <h1>Casual battle</h1>
        <div class="col-12">
            <?php echo "<h2 id='defenderData' " . (!isset($_POST["battleId"]) ? "" : ("data-battle-id='" . $_POST["battleId"] . "' ")) . "data-defender-id='" . $defender->id . "'>" . $defender->renderNameWithLevel() . " trees</h2>"; ?>
        ...
        <div class="col-12 mt-5">
            <div class="row">
                ...
                <div class="col-3">
                    <?php echo '<a class="btn btn-danger" id="' . (!isset($_POST["battleId"]) ? "attackBtn" : "defendBtn") . '">' . (!isset($_POST["battleId"]) ? "Attack" : "Defend") . '!</a>' ?>
                </div>
            </div>
            <table class="table table-striped table-hover mt-3">
                <thead class="table-dark">
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#defendBtn").click(function() {
                ...
                $('#handBody > tr').each(function() {
                    if ($(this).attr("class") == selectedColor) {
                        defendIds += $(this).data("elementId") + "-";
                    }
                });

                if (defendIds != "") {
                    defendIds = defendIds.substring(0, defendIds.length - 1);
                    var battleId = parseInt($("#defenderData").data("battleId"));
                    $.ajax({
                        type: 'POST',
                        url: 'setDefense.php',
                        data: {
